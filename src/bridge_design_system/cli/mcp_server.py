...
@click.command()
@click.option("--port", default=8001, help="Port for the MCP HTTP server")
@click.option(
    "--grasshopper-port", default=8080, help="Port where Grasshopper HTTP server is running"
)
...
def start_mcp_server(port, grasshopper_port, host, debug, reload):
    ...
    try:
        ...
        bridge = GrasshopperMCPBridge(mcp_port=port, grasshopper_port=grasshopper_port)
