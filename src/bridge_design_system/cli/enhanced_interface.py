...
class AgentStatusDisplay:
    ...
    def render_agent_table(self) -> Table:
        ...
        for agent_name, info in self.agent_statuses.items():
            ...
            table.add_row(
                agent_name.title(), Text(status_text, style=color), info["message"][:40], time_str
            )
...
class EnhancedCLI:
    ...
    def create_main_layout(self) -> Layout:
        ...
        layout.split_row(Layout(name="main", ratio=2), Layout(name="sidebar", ratio=1))
        ...
        layout["sidebar"].split_column(
            Layout(name="agents", ratio=1), Layout(name="design", ratio=1)
        )
    ...
    def update_layout(self, layout: Layout, last_response: Optional[str] = None):
        ...
        layout["header"].update(
            Panel(
                Align.center(Text("🌉 AR-Assisted Bridge Design System", style="bold blue")),
                border_style="blue",
            )
        )
        ...
        layout["agents"].update(
            Panel(self.agent_display.render_agent_table(), title="🤖 Agents", border_style="cyan")
        )
        ...
        layout["design"].update(
            Panel(design_content, title="🏗️  Design State", border_style="magenta")
        )
            ...
            with Live(layout, refresh_per_second=2, screen=True) as live:
                while self.running:
                    ...
                    try:
                        ...
                        # Process command
                        should_continue, command_response = self.command_processor.process_command(
                            user_input
                        )
                        ...
                        if user_input:  # Regular design request
                            ...
                            self.agent_display.update_agent_status(
                                "triage", "thinking", f"Analyzing: {user_input[:30]}..."
                            )
                            ...
                            # Reset agent status
                            self.agent_display.update_agent_status(
                                "triage", "idle", "Ready for next task"
                            )
