...
class SimpleAgentCLI:
    ...
    def show_welcome(self):
        ...
        terminal_info = (
            ...
            else f"Modern Terminal ({self.console.color_system})"
        )
        ...
        if self.use_simple_format:
            ...
            self.console.print(
                "🔵 [bold blue]Triage[/bold blue] | 🟢 [bold green]Geometry[/bold green] | 🔴 [bold red]Material[/bold red] | 🟠 [bold orange1]Structural[/bold orange1]"
            )
            self.console.print()
    ...
    def show_agent_status(self):
        ...
        if geometry_status.get("agent_type") == "STDIO":
            self.console.print(
                "[green]✓[/green] Using simplified STDIO-only geometry agent (100% reliable)"
            )
        ...
        if platform.system() == "Windows":
            self.console.print(
                "[dim]Note: Pipe cleanup warnings on Windows are harmless and can be ignored[/dim]"
            )
    ...
    def display_agent_thoughts(self, agent_name: str, thought: str):
        ...
        # Agent symbols
        agent_symbols = {"triage": "[T]", "geometry": "[G]", "material": "[M]", "structural": "[S]"}
        agent_symbol = agent_symbols.get(agent_name, "[?]")
        ...
        try:
            while self.running:
                try:
                    ...
                    timestamp = datetime.now().strftime("%H:%M:%S")
                    self.console.print(
                        f"[dim]{timestamp}[/dim] [bold cyan]You:[/bold cyan] {user_input}"
                    )

                    # Show processing indicator
                    self.display_agent_interaction(
                        "triage", "thinking", f"Analyzing: {user_input[:50]}..."
                    )
                    ...
                    if response.success:
                        ...
                        if "bridge" in user_input.lower() or "create" in user_input.lower():
                            self.display_agent_interaction(
                                "triage", "delegating", "Delegating to geometry agent"
                            )
                            self.display_agent_thoughts(
                                "geometry", "I need to create 3D geometry for this bridge design"
                            )
                            self.display_agent_interaction(
                                "geometry", "active", "Creating bridge geometry"
                            )
                            self.display_agent_thoughts(
                                "geometry", "Generating points, curves, and structural elements"
                            )
                            self.display_agent_interaction(
                                "geometry", "idle", "Geometry creation complete"
                            )
                        elif "material" in user_input.lower():
                            self.display_agent_interaction(
                                "triage", "delegating", "Delegating to material agent"
                            )
                            self.display_agent_thoughts(
                                "material", "Let me check our current material inventory"
                            )
                            self.display_agent_interaction(
                                "material", "active", "Checking material inventory"
                            )
                            self.display_agent_thoughts(
                                "material",
                                "We have steel beams, concrete, and tension cables available",
                            )
                            self.display_agent_interaction(
                                "material", "idle", "Material check complete"
                            )
                        elif "structural" in user_input.lower() or "analysis" in user_input.lower():
                            self.display_agent_interaction(
                                "triage", "delegating", "Delegating to structural agent"
                            )
                            self.display_agent_thoughts(
                                "structural", "Running finite element analysis on the bridge design"
                            )
                            self.display_agent_interaction(
                                "structural", "active", "Running structural analysis"
                            )
                            self.display_agent_thoughts(
                                "structural", "Checking stress distribution and load capacity"
                            )
                            self.display_agent_interaction(
                                "structural", "idle", "Analysis complete"
                            )
