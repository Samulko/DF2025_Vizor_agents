...
SESSION_ID = os.environ.get(
    "BRIDGE_SESSION_ID", f'session_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
)
        ...
        for attempt in range(3):
            try:
                ...
                temp_fd, temp_path = tempfile.mkstemp(
                    dir=memory_file.parent, prefix=".tmp_memory_", suffix=".json"
                )
...
def recall(category: Optional[str] = None, key: Optional[str] = None) -> str:
    ...
    if category is None:
        ...
        summary = (
            f"Memory categories available (session: {memory_data.get('session_id', 'unknown')}):\n"
        )
...
# ==================== TRUE SMOL-AGENTS NATIVE SOLUTION (CORRECTED) ====================
#
# CRITICAL FIX: The previous implementation was flawed because it tried to manually
# parse another agent's memory, which doesn't work in managed_agents pattern.
#
# TRUE SOLUTION: Use proper delegation where agents access their own memory.


@tool
def delegate_element_history_query(element_id: str) -> str:
    """
    Delegate element history query to geometry agent using proper smol-agents pattern.

    This implements the TRUE smol-agents native solution. Instead of manually
    parsing memory, we delegate to the geometry agent to retrieve its own history
    using its self-history tool.

    This tool creates the proper delegation pattern from the critique.

    Args:
        element_id: Element identifier to get history for (e.g., "002", "021")

    Returns:
        Instruction for triage agent on how to delegate properly

    Example Usage (CORRECTED):
        # Step 1: Use this tool to understand delegation pattern
        delegation_info = delegate_element_history_query("002")
        # Step 2: Delegate to geometry agent with proper task
        history = geometry_agent("Use your get_my_element_history tool for element '002'")
        # Step 3: Use that history in next task
        result = geometry_agent(f"Based on this history: {history}, what was the original length?")
    """
    return f"""
PROPER DELEGATION PATTERN FOR ELEMENT {element_id}:

1. FIRST TASK - Get History:
   Ask geometry_agent: "Use your get_my_element_history tool to retrieve all your memory about element '{element_id}'"

2. SECOND TASK - Use History:
   Ask geometry_agent: "Based on your memory about element '{element_id}', [answer the user's question]"

This is the TRUE smol-agents native solution:
- ✅ Geometry agent accesses its own memory via self-tool
- ✅ No external manual parsing of agent.memory.steps  
- ✅ LLM reasons over raw memory context
- ✅ Works properly in managed_agents pattern

Example for "What was original length of element 002?":
1. history = geometry_agent("Use get_my_element_history tool for element '002'")
2. answer = geometry_agent(f"Based on this memory: {{history}}, what was the original length of element '002'?")
"""


@tool
def create_two_step_delegation_task(element_id: str, user_question: str) -> str:
    """
    Create proper two-step delegation task for element history queries.

    This tool helps the triage agent create the correct delegation pattern
    following the smol-agents native solution.

    Args:
        element_id: Element identifier (e.g., "002", "021")
        user_question: The user's question about the element

    Returns:
        Properly formatted two-step delegation instructions
    """
    return f"""
TWO-STEP DELEGATION for: "{user_question}"

STEP 1 - History Retrieval:
Task: "Use your get_my_element_history tool to get all your memory about element '{element_id}'"
Delegate to: geometry_agent

STEP 2 - Question Answering:  
Task: "Based on your memory about element '{element_id}', {user_question.lower()}"
Delegate to: geometry_agent

This follows the TRUE smol-agents pattern:
- Agent retrieves its own memory (no external parsing)
- Agent reasons over its own context
- No manual memory.steps manipulation
"""
...
def remember_component(component_id: str, component_type: str, description: str) -> None:
    ...
    """
    value = (
        f"Type: {component_type}, Description: {description}, Created: {datetime.now().isoformat()}"
    )
    ...
    memory_data["memories"]["components"][component_id] = {
        ...
        "timestamp": datetime.now().isoformat(),
        "category": "components",
        "metadata": {"type": component_type, "description": description},
    }
