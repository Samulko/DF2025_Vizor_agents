"""
Surface Adjust Smolagent - Surface Shape Detection and Adjustment Agent
...
class SurfaceAdjustSmolagent:
    ...
    def __init__(self, model_name: str = "surface_adjust", **kwargs):
        """
        Initialize the surface adjust smolagent with MCP connection and custom tools.
        ...
        logger.info("Establishing MCP connection for surface adjust agent...")
        try:
            ...
            self.agent = ToolCallingAgent(
                ...
                model=self.model,
                max_steps=8,
                name="surface_adjust_agent",
                description=self.description,
            )
            
            # Add specialized system prompt from file or default
            custom_prompt = get_surface_adjust_system_prompt()
            self.agent.prompt_templates["system_prompt"] = (
                self.agent.prompt_templates["system_prompt"] + "\n\n" + custom_prompt
            )
            
            logger.info("Surface adjust smolagent initialized successfully")
            
        except Exception as e:
            logger.error(f"Failed to initialize surface adjust smolagent: {e}")
            raise RuntimeError(f"Surface adjust smolagent initialization failed: {e}")
    ...
    def run(self, task: str) -> Any:
        ...
        """
        logger.info(f"Executing surface adjust task: {task[:100]}...")
        
        try:
            result = self.agent.run(task)
            logger.info("Surface adjust task completed successfully")
            return result
            
        except Exception as e:
            logger.error(f"Surface adjust task failed: {e}")
            raise RuntimeError(f"Surface adjust smolagent execution failed: {e}")
    ...
    def __del__(self):
        ...
        try:
            if hasattr(self, "mcp_connection") and self.mcp_connection:
                ...
                logger.info("MCP connection closed for surface adjust smolagent")
        except Exception as e:
            logger.warning(f"Error closing MCP connection: {e}")

def get_surface_adjust_system_prompt() -> str:
    """Get custom system prompt for surface adjust agent from file or use default."""
    current_file = Path(__file__)
    project_root = current_file.parent
    prompt_path = project_root / "system_prompts" / "surface_adjust_agent.md"
...
def create_surface_adjust_agent(model_name: str = "surface_adjust", **kwargs) -> ToolCallingAgent:
    """
    Factory function for creating surface adjust smolagent instances.
    ...
    """
    logger.info("Creating surface adjust smolagent...")
    
    wrapper = SurfaceAdjustSmolagent(model_name=model_name, **kwargs)
    ...
    logger.info("Surface adjust smolagent created successfully")
    return internal_agent

def demo_surface_adjust():
    """
    ...
    """
    print("Starting surface adjust smolagent demonstration...")
    
    try:
        agent = create_surface_adjust_agent()
