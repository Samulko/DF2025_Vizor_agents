...
class HRCAgentSTDIO:
    ...
    def __init__(self, custom_tools: Optional[List] = None, model_name: str = "hrc"):
        ...
        self.stdio_params = StdioServerParameters(
            command="uv", args=["run", "python", "-m", "grasshopper_mcp.bridge"], env=None
        )
        ...
        logger.info(
            f"Initialized {model_name} agent with STDIO-only transport (temperature=0.1 for precise instruction following)"
        )
...
def create_hrc_agent_stdio(
    custom_tools: Optional[List] = None, model_name: str = "hrc"
) -> HRCAgentSTDIO:
