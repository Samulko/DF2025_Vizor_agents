...
class MonitorCallback:
    ...
    def __init__(self, status_tracker: AgentStatusTracker, agent_name: str, 
                 update_cooldown: float = 0.5, high_performance_mode: bool = False):
        ...
        logger.debug(f"🔗 Created lightweight MonitorCallback for {agent_name} (performance_mode={high_performance_mode})")
        ...
        try:
            ...
            if is_completion:
                ...
                def reset_to_ready():
                    ...
                    if time.time() - self._last_completion_time >= self._completion_reset_delay - 0.1:
                        ...
                        try:
                            ...
                            if loop and not loop.is_closed():
                                loop.run_until_complete(self.status_tracker.update_status(
                                    self.agent_name,
                                    status="ready",
                                    current_task=None,
                                    tool_calls=[]
                        ...
                        logger.debug(f"⏭️ Skipping reset for {self.agent_name} - newer completion detected")
    ...
    def _extract_minimal_step_data(self, memory_step: ActionStep, agent: Any) -> Dict[str, Any]:
        ...
        if hasattr(memory_step, "tool_calls") and memory_step.tool_calls:
            tool_calls = [self._simplify_tool_name(str(tc)) for tc in memory_step.tool_calls[:3]]  # Limit to 3
    ...
    def _extract_simple_tools(self, code: str) -> List[str]:
        ...
        known_tools = [
            "final_answer", "geometry_agent", "syslogic_agent", 
            "get_python3_script", "edit_python3_script", "web_search"
        ]
    ...
    def _determine_simple_status(self, step_data: Dict[str, Any], memory_step: ActionStep, agent: Any) -> str:
        ...
        if "triage" in agent_type:
            ...
            if "Structure Validator" in tool_calls or any("validate" in tool.lower() for tool in tool_calls):
                return "validating"
...
class WebSocketManager:
    ...
    async def broadcast(self, data: Dict[str, Any]) -> None:
        ...
        logger.debug(f"📡 Broadcasting to {len(self.connections)} WebSocket connections: {list(data.keys())}")
        ...
        for connection in self.connections:
            try:
                ...
                if hasattr(connection, 'client_state') and connection.client_state.value == 3:  # DISCONNECTED
...
class RemoteMonitorCallback:
    ...
    def __call__(self, memory_step, agent) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.debug(f"📤 Sent status update for {self.agent_name}: {step_data.get('status')}")
            else:
            ...
            if is_completion:
                self._last_completion_time = time.time()
                logger.debug(f"⏰ {self.agent_name} (remote) completion detected, will remain visible for {self._completion_reset_delay}s")
                ...
                def reset_to_ready():
                    ...
                    if time.time() - self._last_completion_time >= self._completion_reset_delay - 0.1:
                        ...
                        try:
                            ...
                            logger.warning(f"❌ Failed to reset remote status for {self.agent_name}: {e}")
                    else:
                        logger.debug(f"⏭️ Skipping remote reset for {self.agent_name} - newer completion detected")
    ...
    def _extract_tool_calls_from_code(self, code: str) -> list:
        ...
        for match in matches:
            if ("_" in match or 
                match.endswith(("tool", "search", "get", "edit", "component")) or
                match in ["final_answer", "geometry_agent", "syslogic_agent"]):
    ...
    def _determine_simple_status(self, step_data: dict, memory_step, agent) -> str:
        ...
        if any("final_answer" in str(tool) for tool in tool_calls):
            logger.debug(f"🎯 {self.agent_name} (remote) detected completion - tool_calls: {tool_calls}")
...
def create_remote_monitor_callback(agent_name: str, monitoring_url: str = "http://localhost:5000") -> RemoteMonitorCallback:
