...
from bridge_design_system.tools.memory_tools import (
    ...
    load_memory, save_memory, get_memory_file, SESSION_ID
)
...
class TestMemoryConcurrency:
    ...
    def test_concurrent_writes(self, temp_memory_dir):
        ...
        def write_memories(thread_id):
            ...
            for i in range(writes_per_thread):
                try:
                    result = remember(f"thread_{thread_id}", f"key_{i}", f"Thread {thread_id} value {i}")
        ...
        assert total_actual == total_expected, f"Expected {total_expected} memories, got {total_actual}"
    ...
    def test_rapid_session_switching(self, temp_memory_dir):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            all_errors = [error for future in concurrent.futures.as_completed(futures) 
...
class TestLargeDataHandling:
    ...
    def test_memory_file_growth(self, temp_memory_dir):
        ...
        assert late_avg < early_avg * 2, f"Performance degraded from {early_avg:.3f}s to {late_avg:.3f}s"
...
class TestPerformanceUnderLoad:
    ...
    def test_search_performance_degradation(self, temp_memory_dir):
        ...
        for batch_size in batch_sizes:
            # Add batch of data
            for i in range(batch_size):
                remember("search_perf", f"item_{total_items + i}", 
        ...
        if len(search_times) >= 3:
            ...
            if search_times[-1] > search_times[0] * 10:
                pytest.fail(f"Search performance degraded from {search_times[0]:.3f}s to {search_times[-1]:.3f}s")
...
class TestMemoryCleanup:
    ...
    def test_old_session_cleanup(self, temp_memory_dir):
        ...
        for i in range(10):
            ...
            session_data = {
                ...
                "memories": {"test": {"key": {"value": f"Old data {i}", "timestamp": "2023-01-01"}}}
            }
    ...
    def test_memory_growth_monitoring(self, temp_memory_dir):
        ...
        if len(sizes) >= 5:
            ...
            assert late_growth < early_growth * 2.5, f"Memory growth not linear: {early_growth} vs {late_growth}"