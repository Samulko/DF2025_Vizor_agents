...
class TestBasicCreationAndTracking(MemoryTestCase):
    ...
    def test_create_single_bridge_curve(self):
        ...
        self.assertGreater(
            final_state["triage_memory_steps"], 0, "Triage agent should have memory steps"
        )
        ...
        self.assertGreater(
            mock_summary["total_components"], 0, "Should have created at least one component"
        )
    ...
    def test_create_multiple_components_sequential(self):
        ...
        self.assertGreaterEqual(
            final_state["recent_components_count"], 2, "Should track multiple components"
        )
    ...
    def test_component_id_persistence(self):
        ...
        response1 = self.config.simulate_user_request("Create a bridge arch with 8-meter height")
        ...
        response2 = self.config.simulate_user_request("Show me the current component status")
        ...
        self.assertGreaterEqual(
            len(final_components), len(initial_components), "Component tracking should persist"
        )
    ...
    def test_memory_tools_functionality(self):
        ...
        response3 = self.config.simulate_user_request("Debug the current component tracking state")
        ...
        self.assertGreater(
            final_state["triage_memory_steps"], 2, "Should have multiple memory steps"
        )
        self.assertGreater(
            final_state["recent_components_count"], 0, "Should have tracked components"
        )
    ...
    def test_component_creation_with_validation(self):
        ...
        for i, request in enumerate(test_requests):
            ...
            print(
                f"State after request {i+1}: recent_components={state['recent_components_count']}"
            )
        ...
        # Assertions
        self.assertGreaterEqual(
            mock_summary["total_components"], 3, "Should have created at least 3 components"
        )
        self.assertGreater(
            final_state["recent_components_count"], 0, "Should track recent components"
        )
        self.assertEqual(
            mock_summary["components_with_errors"], 0, "No components should have errors"
        )
