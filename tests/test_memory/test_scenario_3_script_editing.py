...
class TestScriptEditingWorkflow(MemoryTestCase):
    ...
    def test_create_edit_verify_workflow(self):
        ...
        self.assertGreater(
            after_edit["triage_memory_steps"],
        ...
        self.assertEqual(
            final_components,
            components_after_create,
            "Edit should modify existing, not create new component",
        )

        # Should have accumulated memory
        self.assertGreaterEqual(
            final_state["triage_memory_steps"], 3, "Should have memory from all three steps"
        )
    ...
    def test_script_error_introduction_and_fixing(self):
        ...
        self.assertEqual(
            initial_summary["components_with_errors"], 0, "Should start with no errors"
        )
        ...
        self.assertEqual(after_fix["components_with_errors"], 0, "Should have no errors after fix")
    ...
    def test_multiple_script_edits_different_components(self):
        ...
        for i, edit_request in enumerate(edits):
            ...
            self.assertEqual(
                current_summary["total_components"],
                component_counts[-1],
    ...
    def test_script_edit_with_vague_reference(self):
        ...
        self.assertGreaterEqual(
            final_summary["total_components"], 1, "Should have at least the original script"
        )
        self.assertLessEqual(
            final_summary["total_components"], 2, "Should not have created many new components"
        )

        # Memory should show progression
        self.assertGreaterEqual(
            final_state["triage_memory_steps"], 3, "Should have memory from create + 2 edits"
        )
