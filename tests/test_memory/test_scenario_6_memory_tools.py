...
class TestMemoryToolsValidation(MemoryTestCase):
    ...
    def test_get_most_recent_component_basic(self):
        ...
        self.assertIn(
            "arch", recent_response.lower(), "Should reference the most recent component (arch)"
        )
    ...
    def test_debug_component_tracking_tool(self):
        ...
        self.assertIn(
            "component", debug_response.lower(), "Debug should show component tracking info"
        )
    ...
    def test_track_geometry_result_tool(self):
        ...
        self.assertGreater(
            after_geometry["recent_components_count"], 0, "Should have tracked geometry components"
        )
        ...
        self.assertIn("track", track_response.lower(), "Should confirm component tracking")
    ...
    def test_memory_tools_integration_workflow(self):
        ...
        self.assertIn("point", response4.lower(), "Should reference most recent component (points)")
        ...
        self.assertGreaterEqual(
            final_state["triage_memory_steps"], 6, "Should have memory from complete workflow"
        )
    ...
    def test_memory_tools_with_errors(self):
        ...
        self.assertGreater(
            error_summary["components_with_errors"], 0, "Should track components with errors"
        )
        ...
        self.assertIn(
            "script", recent_response.lower(), "Should reference recent component even with errors"
        )
        ...
        final_state = self.config.get_memory_state()
        self.assertGreaterEqual(
            final_state["triage_memory_steps"], 5, "Should handle error workflow with memory tools"
        )
    ...
    def test_memory_tools_cross_agent_consistency(self):
        ...
        self.assertGreater(
            memory_state["triage_memory_steps"], 0, "Triage agent should have memory"
        )
        self.assertGreater(
            memory_state["geometry_memory_steps"], 0, "Geometry agent should have memory"
        )
        ...
        final_state = self.config.get_memory_state()
        self.assertGreaterEqual(
            final_state["triage_memory_steps"], 4, "Should maintain cross-agent memory consistency"
        )
