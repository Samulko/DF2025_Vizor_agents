...
class RealModelTestConfig:
    ...
    def __init__(self):
        ...
        try:
            self.real_triage_model = ModelProvider.get_model(
                "triage"
            )  # Real Gemini Flash from .env
            self.real_geometry_model = ModelProvider.get_model(
                "geometry"
            )  # Real Gemini Flash from .env
            logger.info(
                "✅ Real Gemini 2.5 Flash models loaded successfully from .env configuration"
            )
    ...
    def execute_real_model_request(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(
                vague in user_input.lower()
                for vague in ["that", "it", "them", "the curve", "the bridge"]
            ):
    ...
    def get_real_memory_state(self) -> Dict[str, Any]:
        ...
        try:
            memory_steps = 0
            if hasattr(self.real_triage_agent, "memory") and hasattr(
                self.real_triage_agent.memory, "steps"
            ):
            ...
            if (
                hasattr(self.real_triage_agent, "managed_agents")
                and self.real_triage_agent.managed_agents
            ):
            ...
            return {
                ...
                "recent_components": (
                    self.mock_components_created[-5:] if self.mock_components_created else []
                ),
                "total_requests": self.performance_metrics["total_requests"],
            }
        except Exception as e:
            logger.warning(f"Failed to get memory state: {e}")
            return {"has_persistent_context": False, "is_functional": False, "error": str(e)}
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vague_reference_success_rate": (
                1.0 if self.performance_metrics["vague_reference_attempts"] else 0.0
            ),
