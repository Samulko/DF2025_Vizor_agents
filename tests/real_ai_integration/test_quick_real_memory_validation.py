...
def test_quick_real_memory_validation():
    ...
    try:
        ...
        assert response1["success"], f"Basic AI call failed: {response1.get('message', 'Unknown error')}"
        ...
        assert "model" in response1["message"].lower() or "google" in response1["message"].lower() or \
               "confirm" in response1["message"].lower() or "working" in response1["message"].lower(), \
        ...
        assert response2["success"], f"Vague reference call failed: {response2.get('message', 'Unknown error')}"
        logger.info(f"✅ Vague reference call: {response2['latency']:.2f}s")
        
        # Final memory check
        memory_state2 = config.get_real_memory_state()
        assert memory_state2["triage_memory_steps"] >= memory_state["triage_memory_steps"], \
