...
def test_core_issue_modify_curve_you_just_drew():
    ...
    try:
        ...
        for i, user_input in enumerate(conversation):
            ...
            logger.info(f"  🧠 Memory: {memory_state['triage_memory_steps']} steps, functional: {memory_state['is_functional']}")
        ...
        memory_indicators = ["curve", "modify", "arch", "bridge", "geometry", "component", "created", "previous"]
        ...
        assert any(word in third_response for word in ["tall", "height", "20", "percent", "modify", "curve"]), \
        ...
        assert len(found_indicators) >= 2, "Should find multiple memory indicators showing AI understood context"
        assert final_memory['triage_memory_steps'] >= 3, "Should accumulate memory across conversation"
...
def test_extended_vague_reference_patterns():
    ...
    try:
        ...
        for i, pattern in enumerate(test_patterns):
            logger.info(f"🧪 Testing pattern {i+1}/{len(test_patterns)}: {pattern['vague_reference']}")
            ...
            logger.info(f"  ✅ Pattern {i+1}: {len(found_keywords)}/{len(pattern['expected_keywords'])} keywords found")
        ...
        assert success_rate >= 0.67, f"At least 67% of vague reference patterns should work. Got {success_rate:.1%}"
