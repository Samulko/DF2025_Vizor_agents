...
def test_core_vague_reference_simple():
    ...
    try:
        ...
        logger.info(
            f"🧠 Memory state: {memory_state['total_requests']} requests, functional: {memory_state['is_functional']}"
        )
        ...
        logger.info(
            f"  AI understood vague reference: {test_results['detailed_analysis']['vague_reference_understood']}"
        )
        ...
        assert (
            criteria_met >= 2
        ), f"Core vague reference test failed. Only {criteria_met}/3 criteria met. Found indicators: {found_indicators}"
