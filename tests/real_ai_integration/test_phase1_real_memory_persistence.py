...
class TestRealMemoryPersistence:
    ...
    def test_real_memory_persistence_across_model_calls(self, real_ai_config):
        ...
        assert response1[
            "success"
        ], f"First real AI call failed: {response1.get('message', 'Unknown error')}"
        ...
        assert memory_state1[
            "has_persistent_context"
        ], "Real AI should have memory after first call"
        assert memory_state1["is_functional"], "Memory system should be functional"

        logger.info(
            f"✅ First call: {response1['latency']:.2f}s, Memory steps: {memory_state1['triage_memory_steps']}"
        )
        ...
        assert response2[
            "success"
        ], f"Second real AI call failed: {response2.get('message', 'Unknown error')}"
        ...
        assert any(
            word in response_text for word in ["curve", "modify", "taller", "height"]
        ...
        assert memory_state2[
            "has_persistent_context"
        ], "Real AI should maintain memory across calls"
        assert (
            memory_state2["triage_memory_steps"] >= memory_state1["triage_memory_steps"]
        ), "Memory should accumulate across real AI calls"

        logger.info(
            f"✅ Second call: {response2['latency']:.2f}s, Memory steps: {memory_state2['triage_memory_steps']}"
        )
        ...
        assert response3[
            "success"
        ], f"Third real AI call failed: {response3.get('message', 'Unknown error')}"
        ...
        assert memory_state3[
            "has_persistent_context"
        ], "Real AI should maintain memory in longer conversation"
        assert (
            memory_state3["triage_memory_steps"] >= memory_state2["triage_memory_steps"]
        ), "Memory should continue accumulating"

        logger.info(
            f"✅ Third call: {response3['latency']:.2f}s, Memory steps: {memory_state3['triage_memory_steps']}"
        )
    ...
    def test_real_vague_reference_resolution_core_issue(self, real_ai_config):
        ...
        logger.info(
            "🔥 Testing CORE ISSUE: 'modify the curve you just drew' with REAL Gemini 2.5 Flash"
        )
        ...
        for i, user_input in enumerate(conversation):
            ...
            assert (
                response["latency"] < 30.0
            ), f"Real AI call {i+1} too slow: {response['latency']}s"
            ...
            logger.info(
                f"  ✅ Call {i+1}: {response['latency']:.2f}s, Memory: {memory_state['triage_memory_steps']} steps"
            )

        # CRITICAL VALIDATION: \"Modify the curve you just drew\" should work
        curve_modify_response = responses[1]["message"].lower()
        assert any(
            word in curve_modify_response for word in ["curve", "modify", "arch", "bridge"]
        ...
        for i, response in enumerate(responses):
            ...
            if "modify the curve" in user_input.lower():
                ...
                assert any(
                    word in response_text for word in ["connect", "support", "bridge"]
                ), f"Failed to resolve 'them' in call {i+1}"
            elif "fix that error" in user_input.lower():
                assert any(
                    word in response_text for word in ["fix", "error", "correct"]
    ...
    def test_real_ai_memory_under_token_constraints(self, real_ai_config):
        ...
        for i, user_input in enumerate(long_conversation):
            ...
            logger.info(
                f"  ✅ Element {i+1}: {response['latency']:.2f}s, Memory: {memory_state['triage_memory_steps']} steps"
            )
        ...
        assert final_response[
            "success"
        ], f"Long-range memory test failed: {final_response.get('message', 'Unknown error')}"
        ...
        assert any(
            indicator in response_text for indicator in memory_indicators
        ...
        assert final_memory[
            "has_persistent_context"
        ], "Memory should persist through long conversation"
        assert (
            final_memory["triage_memory_steps"] >= 10
        ), "Memory should accumulate across long conversation"
        ...
        ), "At least 90% of calls should succeed under token constraints"
        assert (
            final_memory["triage_memory_steps"] >= 5
        ), "Should maintain substantial memory across conversation"
