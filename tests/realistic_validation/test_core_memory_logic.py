...
from bridge_design_system.tools.memory_tools import remember, recall, search_memory, clear_memory, load_memory
...
class TestMemoryToolFunctionality:
    ...
    def test_memory_tools_basic_functionality(self):
        ...
        # Test memory storage
        result = remember("components", "test_curve_001", f"Type: {mock_component['type']}, Description: {mock_component['description']}")
    ...
    def test_memory_persistence_across_operations(self):
        ...
        for comp in components:
            result = remember("components", comp["id"], f"Type: {comp['type']}, Description: {comp['desc']}")
            assert "Remembered" in result
    ...
    def test_vague_reference_context_storage(self):
        ...
        remember("components", "curve_123", "Type: curve, Description: bridge arch curve, Status: recently_created")
    ...
    def test_memory_categories_work_correctly(self):
        ...
        for category, data in categories_test_data.items():
            for key, expected_value in data.items():
                ...
                assert any(word in retrieved for word in expected_value.split()[:3])  # Check first 3 words
    ...
    def test_memory_search_functionality(self):
        ...
        result_lines = [line for line in lines if line.strip() and not line.startswith('Found') and not line.startswith('(Search')]
...
class TestComponentRegistryLogic:
    ...
    def test_vague_reference_resolution_logic(self):
        ...
        components = [
            {"id": "curve_001", "type": "curve", "name": "Bridge Arch", "description": "main bridge arch"},
            {"id": "support_001", "type": "support", "name": "Steel Beam", "description": "vertical support beam"},
            {"id": "bridge_001", "type": "bridge_structure", "name": "Main Bridge", "description": "complete bridge structure"}
        ]
        ...
        for vague_ref, expected in test_references:
            ...
            if isinstance(expected, list):
                ...
                if expected:  # If we expect specific IDs
                    assert any(exp_id in resolved for exp_id in expected), \
...
class TestErrorHandling:
    ...
    def test_duplicate_component_handling(self):
        ...
        # Register component
        success1 = registry.register_component("test_001", "beam", "Test Beam", "First registration")
        assert success1 == True
        
        # Try to register same ID again
        success2 = registry.register_component("test_001", "post", "Test Post", "Second registration")
