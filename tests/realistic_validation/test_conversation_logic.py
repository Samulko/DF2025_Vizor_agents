...
class MockTriageAgent:
    ...
    def delegate_to_geometry(self, task: str) -> MockDelegatedTask:
        ...
        delegation_context["conversation_history"] = self.conversation_history[
            -3:
        ]  # Last 3 interactions
        ...
        return MockDelegatedTask(
            original_request=task, context=delegation_context, target_agent="geometry"
        )
...
class MockConversationAgent:
    ...
    def process(self, user_input: str) -> MockAgentResponse:
        ...
        if "create" in user_input.lower() and (
            "bridge" in user_input.lower() or "structure" in user_input.lower()
        ):
        ...
        elif "modify" in user_input.lower() and any(
            ref in user_input.lower() for ref in ["curve", "it", "that", "just drew"]
        ):
            ...
            "add" in user_input.lower() and any(ref in user_input.lower() for ref in ["arch", "it"])
        ) or (
            "make" in user_input.lower()
            and any(ref in user_input.lower() for ref in ["arch", "curved", "curve"])
        ):
            ...
            "taller" in user_input.lower()
            or "height" in user_input.lower()
            or "adjust" in user_input.lower()
        ):
            # Height adjustment
            if "last_created" in self.conversation_state:
                target_component = self.conversation_state["last_created"]
        ...
        elif "connect" in user_input.lower() and (
            "support" in user_input.lower() or "it" in user_input.lower()
        ):
...
class TestAgentDelegationLogic:
    ...
    def test_delegation_with_conversation_history(self):
        ...
        assert any(
            "curve" in str(item).lower() for item in delegated_task.context["conversation_history"]
        )
    ...
    def test_context_enrichment_from_memory(self):
        ...
        assert (
            "relevant_components" in delegated_task.context
            or "recent_context" in delegated_task.context
        )
